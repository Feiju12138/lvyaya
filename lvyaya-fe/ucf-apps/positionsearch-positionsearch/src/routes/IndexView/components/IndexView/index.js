...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "positionsearch", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {positionsearchObj, positionsearchIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = positionsearchObj.list[positionsearchIndex] || {};
    ...
    onPageSelect = (value, type, tableName) => {
        ...
        if (tableName === "positionsearchObj") { //主表分页
            searchParam.pageSize = pageSize;
            searchParam.pageIndex = pageIndex;
            actions.masterDetailMany.loadList(searchParam);
        } else {
            //子表分页
            const {positionsearchIndex, positionsearchObj} = this.props;
            const {id: search_NA} = positionsearchObj.list[positionsearchIndex];
    ...
    onPrint = () => {
        const {positionsearchIndex, positionsearchObj} = this.props;
        const {list} = positionsearchObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[positionsearchIndex] || {};
        actions.masterDetailMany.printDocument({
            queryParams: {
                funccode: 'positionsearch',
                nodekey: 'positionsearch'
            },
            printParams: {id: id}
        });
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "positionsearch") { // 主表
                const {positionsearchIndex} = this.props;
                const record = list[positionsearchIndex];
                await actions.masterDetailMany.delpositionsearch(record);
            }
        }
    }
    /**
    ...
    render() {
        ...
        let selectRow = positionsearchObj['list'][positionsearchIndex] || {};
        // 主表数据为空
        const positionsearchForbid = positionsearchObj.list.length > 0 ? false : true;
        let {list} = positionsearchObj;
        const {id} = list[positionsearchIndex] || {};

        return (
            <div className='master-detail-many'>
                <Header title='垃圾站位置查询'/>
                <SearchArea positionsearchObj={positionsearchObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="positionsearch">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('positionsearch', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={positionsearchForbid}
                            onClick={() => _this.onShowMainModal("positionsearch", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={positionsearchForbid}
                            onClick={() => _this.onShowMainModal("positionsearch", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={positionsearchForbid}
                            onClick={() => _this.onClickDel("positionsearch")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ref="positionsearch"
                    data={positionsearchObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.positionsearchColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({positionsearchIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            positionsearchObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return positionsearchIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(positionsearchObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "positionsearchObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "positionsearchObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={positionsearchForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
