...
class AddEditpositionsearch extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { positionsearchObj, positionsearchIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "positionsearch") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.savepositionsearch(values); //保存主表数据
                //返回
    ...
    render() {
        ...
        <FormControl disabled={typeof btnFlag != 'undefined' && btnFlag == 2
            ...
            ...getFieldProps('address', {
                ...
                initialValue: (typeof rowData === 'undefined' || typeof rowData.address === 'undefined') ? "" : rowData.address
,
                rules: [{
                    type:'string',required: false,pattern:/\S+/ig, message: '请输入地址',
                }],
                onChange(value) {
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        <FormControl disabled={typeof btnFlag != 'undefined' && btnFlag == 2
            ...
            ...getFieldProps('city', {
                validateTrigger: 'onBlur',
                initialValue: (typeof rowData === 'undefined' || typeof rowData.city === 'undefined') ? "" : rowData.city
,
                rules: [{
                    type:'string',required: true,pattern:/\S+/ig, message: '请输入城市',
                }],
                onChange(value) {
if(typeof rowData !== 'undefined'){
    let tempRow = Object.assign({},rowData,{ city: value });
    _this.setState({
        rowData:tempRow
    })
}
        ...
        />
        <FormError errorMsg={getFieldError('city')}/>
                                </FormItem>
                                <FormItem>
        <Label >
            街道
        ...
        <FormControl disabled={typeof btnFlag != 'undefined' && btnFlag == 2
            ...
            ...getFieldProps('street', {
                validateTrigger: 'onBlur',
                initialValue: (typeof rowData === 'undefined' || typeof rowData.street === 'undefined') ? "" : rowData.street
,
                rules: [{
                    type:'string',required: false,pattern:/\S+/ig, message: '请输入街道',
                }],
                onChange(value) {
if(typeof rowData !== 'undefined'){
    let tempRow = Object.assign({},rowData,{ street: value });
    _this.setState({
        rowData:tempRow
    })
}
        ...
        />
        <FormError errorMsg={getFieldError('street')}/>
                                </FormItem>
                </FormList>
            </div>
        )
...
export default FormList.createForm()(AddEditpositionsearch);
