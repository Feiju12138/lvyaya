...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "textsearch", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {textsearchObj, textsearchIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = textsearchObj.list[textsearchIndex] || {};
    ...
    onPrint = () => {
        const {textsearchIndex, textsearchObj} = this.props;
        const {list} = textsearchObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[textsearchIndex] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "textsearch") { // 主表
                const {textsearchIndex} = this.props;
                const record = list[textsearchIndex];
                await actions.masterDetailMany.deltextsearch(record);
            }
        }
    }
    /**
    ...
    textsearchColumn = [
        {
            title: "垃圾名称",
            dataIndex: "rubbish_name",
            key: "rubbish_name",
            width: 150,
        },
        {
            title: "垃圾种类",
            dataIndex: "rubbish_kind",
            key: "rubbish_kind",
            width: 150,
        },
    ];
    ...
    render() {
        ...
        let selectRow = textsearchObj['list'][textsearchIndex] || {};
        // 主表数据为空
        const textsearchForbid = textsearchObj.list.length > 0 ? false : true;
        let {list} = textsearchObj;
        const {id} = list[textsearchIndex] || {};

        return (
            <div className='master-detail-many'>
                <Header title='文字检索'/>
                <SearchArea textsearchObj={textsearchObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="textsearch">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('textsearch', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={textsearchForbid}
                            onClick={() => _this.onShowMainModal("textsearch", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={textsearchForbid}
                            onClick={() => _this.onShowMainModal("textsearch", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={textsearchForbid}
                            onClick={() => _this.onClickDel("textsearch")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ref="textsearch"
                    data={textsearchObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.textsearchColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({textsearchIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            textsearchObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return textsearchIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(textsearchObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "textsearchObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "textsearchObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={textsearchForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
