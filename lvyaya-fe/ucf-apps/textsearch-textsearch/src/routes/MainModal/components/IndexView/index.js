...
class AddEdittextsearch extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { textsearchObj, textsearchIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "textsearch") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.savetextsearch(values); //保存主表数据
                //返回
    ...
    render() {
        ...
        <FormControl disabled={typeof btnFlag != 'undefined' && btnFlag == 2
            ...
            {
            ...getFieldProps('rubbish_name', {
                validateTrigger: 'onBlur',
                initialValue: (typeof rowData === 'undefined' || typeof rowData.rubbish_name === 'undefined') ? "" : rowData.rubbish_name
,
                rules: [{
                    type:'string',required: true,pattern:/\S+/ig, message: '请输入垃圾名称',
                }],
                onChange(value) {
if(typeof rowData !== 'undefined'){
    let tempRow = Object.assign({},rowData,{ rubbish_name: value });
    _this.setState({
        rowData:tempRow
    })
}
        ...
        />
        <FormError errorMsg={getFieldError('rubbish_name')}/>
                                </FormItem>
                                <FormItem>
        <Label >
            垃圾种类
        ...
        <FormControl disabled={typeof btnFlag != 'undefined' && btnFlag == 2
            ...
            {
            ...getFieldProps('rubbish_kind', {
                validateTrigger: 'onBlur',
                initialValue: (typeof rowData === 'undefined' || typeof rowData.rubbish_kind === 'undefined') ? "" : rowData.rubbish_kind
,
                rules: [{
                ...
                onChange(value) {
if(typeof rowData !== 'undefined'){
    let tempRow = Object.assign({},rowData,{ rubbish_kind: value });
    _this.setState({
        rowData:tempRow
    })
}
        ...
        />
        <FormError errorMsg={getFieldError('rubbish_kind')}/>
                                </FormItem>
                </FormList>
            </div>
        )
...
export default FormList.createForm()(AddEdittextsearch);
