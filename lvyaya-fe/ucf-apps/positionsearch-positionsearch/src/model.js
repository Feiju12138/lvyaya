...
export default {
    ...
    effects: {
        ...
        async loadList(param, getState) {
            ...
            const {result} = processData(await api.getpositionsearch(param));  // 调用 getList 请求数据
            const {data: respositionsearch} = result;
            const {content = []} = respositionsearch || {};

            if (content.length > 0) { // 获取子表数据
                const positionsearchObj = structureObj(respositionsearch, param);
                actions.masterDetailMany.updateState({positionsearchObj}); // 更新主表数据
            } else {
                 const {
                    positionsearchObj,
                } = getState().masterDetailMany;
                actions.masterDetailMany.updateState({   // 如果请求出错,数据初始化
                        positionsearchObj: initStateObj(positionsearchObj),
                        positionsearchRow: {}
                    }
                );
            }
            actions.masterDetailMany.updateState({showLoading: false, positionsearchIndex: 0});
        },
        /**
         * getSelect：保存主表数据
         * @param {*} param
         * @param {*} getState
         */
        async savepositionsearch(param, getState) {
            actions.masterDetailMany.updateState({showLoading: true});   // 正在加载数据，显示加载 Loading 图标
            const {btnFlag} = param;
            let status = null;
            delete param.btnFlag; //删除标识字段
            if (btnFlag === 0) { // 添加数据
                const {result} = processData(await api.savepositionsearch(param), '添加成功');
                status = result.status;
            }
            if (btnFlag === 1) { // 修改数据
                const {result} = processData(await api.updatepositionsearch(param), '修改成功');
                status = result.status;
            }

            if (status === 'success') { // 如果不判断是会报错，param参数有错
                const {pageSize} = getState().masterDetailMany.positionsearchObj;
                // 带上子表信息
         ...
         * @param {*} getState
         */
        async delpositionsearch(param, getState) {
            const {id} = param;
            const {result}=processData(await api.delpositionsearch([{id}]), '删除成功');
            const {status}=result;
            if(status==='success'){
                // 获取表pageSize;
                const {positionsearchObj} = getState().masterDetailMany;
                const {pageSize} = positionsearchObj;
         ...
         * @param {*} getState
         */
        async querypositionsearch(param, getState) {
